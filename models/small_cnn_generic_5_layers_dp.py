...
class Small_CNN_Generic_5_layers_dp(Small_CNN_Generic_2_layers):
    ...
    def __init__(self, first_layer_kernel_size, second_layer_kernel_size, channels, image_resolution: int, dilation: int = 1, stride: int = 1, dropout_p: float = 0.25):
        super(Small_CNN_Generic_5_layers_dp, self).__init__(first_layer_kernel_size, second_layer_kernel_size, channels, image_resolution, dilation, stride)
        ...
        self.cnn3 = nn.Conv2d(in_channels=channels[2], out_channels=channels[3], kernel_size=default_kernel_size, stride=stride, padding=padding)
        self.relu3 = nn.ReLU(inplace=True)
        self.dropout3 = nn.Dropout2d(p=dropout_p)
        self.maxpool3=nn.MaxPool2d(kernel_size=2)

        padding = int(default_kernel_size / 2)
        self.cnn4 = nn.Conv2d(in_channels=channels[3], out_channels=channels[4], kernel_size=default_kernel_size, stride=1, padding=padding)
        self.relu4 = nn.ReLU(inplace=True)
        self.dropout4 = nn.Dropout2d(p=dropout_p)
        self.maxpool4=nn.MaxPool2d(kernel_size=2)

        self.cnn5 = nn.Conv2d(in_channels=channels[4], out_channels=channels[4], kernel_size=default_kernel_size, stride=1, padding=padding)
